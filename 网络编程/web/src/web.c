...
{
	//zlog_info(g_cate, "..handle_sigpipe...%d", sig);
	return;
}
...
static void post_dynamic(int connfd, const char *filename, char *post_data, int content_length)
{
	...
	if (Fork() == 0)
	{
		...
		zlog_info(g_cate, post_data);	
		Rio_writen(pipe_in[1], post_data, content_length);
		zlog_info(g_cate, "child end");
		exit(0);
	}
	//! parent	
	//else 
	//{
		...
		zlog_info(g_cate, "execve start");
		Execve(filename, emptylist, environ);	
		zlog_info(g_cate, "execve error");		
	//}
#endif
}
...
void doit(int connfd)
{
	...
	if (strlen(buf) <= 3)
	{
		zlog_info(g_cate, "doit no content");
		goto finish;
	}
