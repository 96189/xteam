...
int rc = 0;
zlog_category_t *g_cate = NULL;

#ifdef HTTPS
#include <openssl/ssl.h>
#include <openssl/err.h>

#define DIGIT_CERT "./conf/cacert.pem"
#define USER_PRIVATE_KEY "./conf/privkey.pem"
//ssl
SSL_CTX *g_ctx = NULL;
SSL *g_ssl = NULL;
...
int main(int argc, char *argv[])
{
	...
	if (rc)
	{
		printf("zlog_init failed log:%s\n", my_conf.logfile);
		return -1;
	}

	g_cate = zlog_get_category(my_conf.category);
	if (!g_cate)
	{
		printf("get cat fail\n");
		zlog_fini();
	...
	while (1)
	{
		...
		if (connfd > 0)		
		{
			zlog_info(g_cate, "connfd=%d", connfd);
			doit(connfd);
		}
