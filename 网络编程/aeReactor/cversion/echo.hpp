...
public:
    static void writeHandler(struct aeEventLoop *eventLoop, int fd, void *clientData, int mask)
    {
        int len = 0;
    ...
    static void readHandler(struct aeEventLoop *eventLoop, int fd, void *clientData, int mask)
    {
        char readbuf[1024] = {};
        int len = -1;
        ...
        if ((len = read(fd, readbuf, 1024)) > 0)
        {
            ...
            if (aeCreateFileEvent(eventLoop, fd, AE_WRITABLE, writeHandler, clientData) != AE_OK)
            {
    ...
    static void acceptTcpHandler(struct aeEventLoop *eventLoop, int fd, void *clientData, int mask)
    {
        int acceptfd = -1;
        ...
        if (aeCreateFileEvent(eventLoop, acceptfd, AE_READABLE, readHandler, usrbuf) != AE_OK)
        {
