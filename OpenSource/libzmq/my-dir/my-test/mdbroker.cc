...
void Broker::InsertService (char *name, Service *service)
{
    zhash_insert (this->services_, name, service);
    zhash_freefn (this->services_, name, service->Destroy);
}
void Broker::InsertWorker (char *id_string, Worker *worker)
{
...
Worker *Broker::RequireWorker (zframe_t *identity)
{
    ...
    // 查找
    Worker *worker = (Worker *) zhash_lookup (this->workers_, id_string);
    if (worker == NULL) {
        // 创建
        worker = new Worker (this, id_string, identity);
        InsertWorker (id_string, worker);
        if (this->verbose_) {
...
Service *Broker::RequireService (zframe_t *servicename)
{
    ...
    // 查找
    Service *service = (Service *) zhash_lookup (this->services_, name);
    if (service == NULL) {
        // 创建
        service = new Service (this, name);
        InsertService (name, service);
        if (this->verbose_)
...
void Broker::ProcessWorkerMsg (zframe_t *sender, zmsg_t *msg)
{
    ...
    int worker_ready = (zhash_lookup (this->workers_, id_string) != NULL);
    free (id_string);
...
void Broker::Purge()
{
    Worker *worker = (Worker *) zlist_first (this->waiting_);
    while (worker) {
        ...
        worker->Delete (0);
        worker = (Worker *) zlist_first (this->waiting_);
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        if (zclock_time() > broker->heartbeat_at_)
        {
            ...
            broker->Purge();
            // 给所有有效的worker发送心跳包
            Worker *worker = (Worker*) zlist_first(broker->waiting_);
            while (worker)
            {
                worker->Send(MDPW_HEARTBEAT, NULL, NULL);
                worker = (Worker*)zlist_next(broker->waiting_);
            }
